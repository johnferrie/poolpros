  ...
  return (
    <div className="dealer">
      <div className="dealer__top-content">
        ...
        <a href="tel:" className="dealer__phone">
          {phone1}
        </a>
        <p className="dealer__email-message">
          Can't talk now? Click below to send an email.
        </p>
        <button
          className="dealer__contact-btn"
          onClick={event =>
            props.setModal({ screen: 'email', data: props.data })
          }
        >
          Contact this Pro
        </button>
        <div className="dealer__hours">
          ...
          {hoursMapKeys.length ? (
            <ul className="dealer__hours-list">
              {hoursMapKeys.map(key => (
                <li key={key} className="dealer__hours-open">
                  {capitalizeFirst(key)} {hoursMap[key]}
                </li>
              ))}
            </ul>
          ) : null}
        ...
        <div className="dealer__certifications">
          <ul className="dealer__certifications-list">
            {certifications.map(certification => (
              <li
                key={slugify(certification)}
                className={`dealer__certification dealer__certification--${slugify(
                  certification
                )}`}
              >
                {certification}
